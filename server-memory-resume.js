...
app.post('/voice', async (req, res) => {
  ...
  // Build the Hume URL with resumed parameter if returning caller
  let humeUrl = `https://api.hume.ai/v0/evi/twilio?config_id=${process.env.HUME_CONFIG_ID}&api_key=${process.env.HUME_API_KEY}`;

  if (caller.chat_group_id) {
    humeUrl += `&resumed_chat_group_id=${caller.chat_group_id}`;
    console.log(`   ✨ Resuming chat group: ${caller.chat_group_id}`);
  } else {
    console.log(`   🆕 Starting new chat group`);
  }

  // Generate TwiML that redirects to Hume with resume parameter
...
app.listen(port, () => {
  console.log('\n🚀 Baton AI Memory Server (With Resume Support)');
  ...
  console.log('✨ Ready to track conversations with memory!\n');
});
